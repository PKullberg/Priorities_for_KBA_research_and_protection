# This is a refreshed version of gbif function of dismo package for my own usage. Many arguments of the old version area not workin due to changes(?) in the gbif API
# Author: Peter Kulberg, peter.kullberg@helsinki.fi
# change to test git
...
.fixNameGBIF <- function(genus, species) {
  ...
  gensp <- paste(genus, species) ###### WHy is this here? Some remnant thing?
  spec <- gsub("   ", " ", species) 
  spec <- gsub("  ", " ", spec) 	
...
# genus = name[1], species = name[2], ext = NULL, args = NULL, geo = TRUE, 
#           sp = FALSE, removeZeros = FALSE, download = TRUE, ntries = 5, 
#           nrecs = 300, start = 1, end = Inf


gbif2 <- function (genus, species = "", ext = NULL, args = NULL, geo = TRUE, 
          sp = FALSE, removeZeros = FALSE, download = TRUE, ntries = 5, 
          nrecs = 300, start = 1, end = Inf, temp2 = NA) 
{
  if (!requireNamespace("jsonlite")) {
    stop("You need to install the jsonlite package to use this function")
  }
 
  tmpfile <- paste(tempfile(), ".json", sep = "")
  
   if (!is.na(temp2)) {
    tmpfile <- paste(tempfile(tmpdir = temp2), ".json", sep = "") ### One could use straight tempfile to produce the wanted ending to the file name.
  }
  ...
  url1 <- paste(base, "scientificname=", spec, "&limit=1", 
                cds, ex, args, sep = "")
  ...
  start <- max(1, start)
  stopifnot(start <= end) ### what is this doing here ??
  nrecs <- min(max(nrecs, 1), 300)
  url1 <- paste(base, "scientificname=", spec, "&limit=", format(nrecs, 
                                                                 scientific = FALSE), cds, ex, args, sep = "")
  g <- list()
  breakout <- FALSE
  np <- i <- 1
  while (TRUE) {
    if (start + nrecs >= end) {
      nrecs <- end - start + 1
      url1 <- paste(base, "scientificname=", spec, "&limit=", 
                    format(nrecs, scientific = FALSE), cds, ex, args, 
                    sep = "")
      breakout <- TRUE
    }
    aurl <- paste(url1, "&offset=", format(start - 1, scientific = FALSE), 
                  sep = "")
    if (np > 20) {
      np <- 1
      message("")
    ...
    while (TRUE) {
      ...
      if (class(test) == "try-error") {
        ...
        json <- scan(tmpfile, what = "character", quiet = TRUE, 
                     sep = "\n", encoding = "UTF-8")
        ...
        r <- x$results
        r <- r[, !sapply(r, class) %in% c("data.frame", 
                                          "list")]
        rownames(r) <- NULL
        g[[i]] <- r
        break
  ...
  if (("lat" %in% cn) & ("lon" %in% cn)) {
    ...
    k <- apply(z[, c("lon", "lat")], 1, function(x) isTRUE(any(x == 
                                                                 0)))
    if (removeZeros) {
      if (geo) {
        z <- z[!k, ]
  ...
  if (nrow(z) > 0) {
    ...
    if (length(vrs) > 0) {
      ...
      fullloc <- apply(fullloc, 1, function(x) paste(x, 
                                                     collapse = ", "))
